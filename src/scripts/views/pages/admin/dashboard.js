...
const AdminDashboard = {
  async render() {
    return AdminLayout.render(`
        <div class="p-4 md:p-6 space-y-6 transition-all duration-300 w-full">
          <!-- Stats Overview -->
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mx-auto max-w-full">
            <!-- Total Users -->
            ...
            <!-- Stats cards with similar structure -->
            <div class="bg-white rounded-lg shadow-md p-4 md:p-6 hover:shadow-lg transition-all w-full">
              <div class="flex items-center">
  ...
  async afterRender() {
    try {
      ...
      if (data.latestProducts) this._renderLatestProducts(data.latestProducts);
      if (data.latestBuyOffers) this._renderLatestBuyOffers(data.latestBuyOffers);
